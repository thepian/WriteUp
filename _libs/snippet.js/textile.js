...
function convert(t) {
	...
	for(var i=0,literal=false;i<sections.length;i++) {
		var html_snip = sections[i];
		if (literal) {
			html_snip = html_snip.replace(/&/g,"&amp;").replace(/\r?\n\s/g,"\n&nbsp;").replace(/\r?\n\s\s/g,"\n&nbsp;&nbsp;");
			html_snip = html_snip.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r?\n/g,"<br>\n");
			lines = lines.concat('<code class="blockquote">', html_snip.split(/\r?\n/), '</code>');
	} else {
			lines = lines.concat(html_snip.split(/\r?\n/));
		}
		literal = !literal;
	}
	html="";
	...
	for(i=0;i<lines.length;i++) {
		...
		if(mm=dt.exec(lines[i])){stp(1);html += lines[i].replace(dt,"<dt"+make_attr(mm[1])+">"+prep(mm[2]))+"</dt>";continue;}
		if(mm=dd.exec(lines[i])){stp(1);html += lines[i].replace(dd,"<dd"+make_attr(mm[1])+">"+prep(mm[2]))+"</dd>";continue;}
